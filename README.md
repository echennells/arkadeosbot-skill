...
[Arkade](https://arkade.computer) is a Bitcoin Layer 2 built on the [Ark protocol](https://arkdev.info). It uses Virtual Transaction Outputs (VTXOs) for instant off-chain payments with batched on-chain settlement. It is fully self-custodial -- you hold your own keys via a BIP39 mnemonic. An operator (ASP/Ark Service Provider) batches transactions and settles them on-chain, but cannot steal funds -- users can always exit to L1 unilaterally.
...
- **Low fees** -- Ark-to-Ark transfers are free. On-chain exit costs 200 sats per output.
...
| Network | ASP Server | Delegator |
|---------|-----------|-----------|
| bitcoin | `https://arkade.computer` | `https://delegate.arkade.money` |
| mutinynet | `https://mutinynet.arkade.sh` | `https://delegator.mutinynet.arkade.sh` |
| regtest | `http://localhost:7070` | N/A |
...
**Mnemonic = full wallet access.** An Ark mnemonic can do everything: check balance, send payments, and withdraw to L1. There is no permission scoping, no spending limits, no read-only mode.
...
Lightning (send/receive BOLT11 invoices) is **not included** in this skill. Arkade supports Lightning via [Boltz Exchange](https://boltz.exchange) swaps, but the swap lifecycle is stateful and not suitable for an agent:

- **Receiving** requires generating a preimage and claim keypair, monitoring a WebSocket for payment, then constructing and broadcasting a VHTLC claim transaction -- all before a timeout expires.
...
Both paths require persistent state tracking across multiple asynchronous steps. If the agent crashes or restarts mid-swap, funds can be lost or stuck. This doesn't fit the stateless call-and-return pattern expected of a skill.

If you need Lightning, consider [sparkbtcbot-skill](https://github.com/echennells/sparkbtcbot-skill) which has built-in Lightning support via Spark's native integration -- no swaps or state management required. Alternatively, use the `@arkade-os/boltz-swap` package directly in a long-running process with proper swap state persistence.
